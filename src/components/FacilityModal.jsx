/**
 * src/components/FacilityModal.jsx  —  v2
 * MODIFICHE v2:
 *  - Rimossi i 4 blocchi contatto (Direzione, Dir. Sanitaria,
 *    Ref. Struttura, Ref. Qualità) dal form di creazione/modifica struttura.
 *    Motivazione: i contatti sono gestiti dal Direttore dalla tab
 *    "Panoramica" in DirectorFacility, non dall'admin in fase di setup.
 *    Qui bastano i dati identificativi e logistici.
 *  - EMPTY e facilityToForm aggiornati di conseguenza.
 *  - Il payload inviato a onSave include ora solo i campi del form ridotto.
 *    I campi contatto NON vengono azzerati su strutture esistenti
 *    (il payload non li include, quindi Supabase non li tocca).
 */
...
export default function FacilityModal({ isOpen, onClose, udos, facility, onSave, onDelete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      // Il payload include SOLO i campi del form ridotto.
      // I campi contatto (director, email_direzione, ecc.) non vengono toccati
      // su strutture esistenti; su strutture nuove restano NULL nel DB.
      const payload = {
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3">
                <p className="text-xs font-bold text-indigo-600">
                  ℹ I contatti della struttura (Direttore, Dir. Sanitario, Ref. Qualità ecc.) vengono gestiti dal Direttore dalla sua dashboard, nella tab "Panoramica".
                </p>
              </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
              {facility ? (
                <button type="button" onClick={handleDelete}
                  className="flex items-center gap-2 text-xs font-black text-rose-500 uppercase tracking-widest hover:text-rose-700 transition-colors">
                  <Trash2 size={14} /> Elimina
                </button>
              ) : <div />}
              <div className="flex gap-3">
                <button type="button" onClick={onClose}
                  className="px-6 py-3 rounded-xl text-xs font-black uppercase text-slate-500 hover:bg-slate-200 transition-colors">
                  Annulla
                </button>
                <button type="submit" disabled={saving}
                  className="flex items-center gap-2 bg-indigo-600 text-white px-8 py-3 rounded-xl text-xs font-black uppercase shadow-md hover:bg-indigo-700 transition-colors disabled:opacity-60">
                  <Save size={14} />
